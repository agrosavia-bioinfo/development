...
main <- function (args) {
	...
	phenoStructClass = setPopulationStructure (data2, genotype, structure, gwasModel, phenoStructFile, data3)
...
#-----------------------------------------------		--------------
setPopulationStructure <- function (data2, genotype, structure, gwasModel, phenoStructFile, data3) {
	#setClass ("GWASpolyStruct", slots=c(params="list"), contains="GWASpoly.K")
	phenoStructClass = new ("GWASpolyStruct", data2)

	if (gwasModel=="Structure") {
		msg ("Adding structure...")
		...
		data2@pheno = phenoStruct

		gwasParams = set.params(fixed=structNames, fixed.type=structTypes)
		phenoStructClass@params = gwasParams
	}else
		phenoStructClass@params = NULL

	#params <- set.params(fixed=c("Grp1","Grp2","Grp3","Grp4"), fixed.type=rep("numeric",4))
	#params <- set.params(n.PC=10)

	return (phenoStructClass)
}
