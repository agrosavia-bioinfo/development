...
#
# Sample Usage
# First, let us create some fake data to test it with. We will use an imaginary animal that has 10 autosomes. The random sample data is created with
...
dd = read.csv ("scores.csv", sep="\t")
print (dd[1:10,])

#dd = data[data$TOOL=="SHEsis", c("CHROM", "POSITION", "PVALUE", "SCORE")]
#dd$PVALUE = 10^(-dd$SCORE)
#print (dd)

ann = rep (1, length(dd$PVALUE))

ann [with (dd, TOOL=="GWASpoly")] <-2
ann [with (dd, TOOL=="SHEsis")] <-3
ann [with (dd, TOOL=="PLINK")] <-4
ann [with (dd, TOOL=="TASSEL")] <-5
ann<-factor(ann, levels=1:5) #, labels=c("","GENE1","GENE2", "GENE3", "GENE4"))



manhattan.plot(dd$CHROM, dd$POSITION, 10^(-dd$SCORE), should.thin=F, annotate=ann)
