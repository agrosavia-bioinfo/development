...
createHeatmapForSNPList <- function (outputDir, genoFileACGT, genoFileNUM, phenoFile, snpList) {
	...
	if (length (snpList) == 0) {
		pdf (paste0(outName,"-", "none", ".pdf"), width=7, height=7) 
			plot.new ()
		   	mtext ("No heatmaps (without shared SNPs)")
		dev.off()
		png (paste0(outName,"-","none", ".png"), width=7, height=7, units="in", res=72)
			plot.new ()
		   	mtext ("No heatmaps (without shared SNPs)")
		dev.off()
	}else {
...
createHeatmapForSNP <- function (outputDir, genoFileACGT, genoFileNUM, phenoFile, snpId) {
	...
	#>>>>>>> Local function createHeatmap
	outName = paste0 (outputDir, "/out-SNPProfile") 
	createHeatmap <- function (genoxfenov4, phenoValues, outputType="pdf") {
		if (outputType=="pdf")
			pdf (paste0(outName,"-", snpId, ".pdf"), width=7, height=7)
		else
			png (paste0(outName,"-",snpId, ".png"), width=7, height=7, units="in", res=72)

		plotHistogram = function () hist(phenoValues, main = "Histogram", xlab=trait)
		hmap = heatmap.2(genoxfenov4,dendrogram = "row",reorderfun=function(snpId, w) reorder(snpId, w, agglo.FUN = mean),
				  ...
				  col=my_palette,cexCol = 1,lmat=lmat,lhei=lhei, lwid=lwid, extrafun=plotHistogram, 
				  key.xlab=paste0 ("Value of ", trait), xlab=snpId, key.title="Color Key")
		dev.off()
	}#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<

	createHeatmap (genoxfenov4, phenoValues, "pdf")
	createHeatmap (genoxfenov4, phenoValues, "png")
}
