...
main <- function () {
	...
	genoFileACGT <- "genoACGT.csv"
	genoFileNUM  <- "genoNUM.csv"
	phenoFile    <- "pheno.csv"
	snpList      <- c ("solcap_snp_c1_1")         # SNPs a visualizar
...
createHeatmapForSNP <- function (outputDir, genoFileACGT, genoFileNUM, phenoFile, snpId) {
	...
	print (dim(genotypeACGT))
	print (dim(genotypeNUMERIC))
	print (dim(phenotype))
	...
	hd (genoxfeno_3)
	posNAs = which (is.na (genoxfeno_3))
	print (posNAs)
	...
	write.table (file="outg3.csv", genoxfeno_3)
	print (">>> NAs:")
	print (">>>")

	genoxfenov4<-as.matrix(genoxfeno_3)
	#print (genoxfeno_2 [1:10,])
	#print (genoxfenov4 [1:10,])
	#genoxfenov4<-as.matrix(genoxfeno_3[genoxfeno_3,])
