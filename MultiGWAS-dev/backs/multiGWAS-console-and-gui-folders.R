...
main <- function (args) 
{
	...
	config = initWorkingEnvironment (configFile, outDir)
...
#-------------------------------------------------------------
initWorkingEnvironment <- function (configFile, outDir) {
	...
	# Copy config, phen, and gheno file to workingDir
	runCommand (sprintf ("cp -s %s .", genotypeFile))
	runCommand (sprintf ("cp -s %s .", phenotypeFile))
...
runShesisGwas <- function (params) 
{
	...
	if (params$gwasModel == "Naive") {
		...
		outFile = paste0 (inGeno,"-kinship-shesis")
		cmm    = sprintf ("%s/sources/scripts/script-kinship-plink2.sh %s %s", HOME, inGeno, outFile)
		runCommand (cmm, "log-kinship.log")

		# SHEsis: Filter geno/pheno to individuals, write out, and convert to SHEsis format
		kinshipIndividuals  = read.table (file=paste0(outFile, ".ped"), sep="\t")
