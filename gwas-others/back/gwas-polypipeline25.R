...
#-------------------------------------------------------------
#-------------------------------------------------------------
runGwasPlink <- function () {
	cmm="plink --file filtered-plink-genotype --linear --assoc --adjust --pheno filtered-plink-phenotype.tbl --all-pheno --allow-no-sex --out out-plink"
	system2 (cmm)
}
...
dataPreprocessing <- function (genotypeFile, phenotypeFile, structureFile) {
	...
	filteredMarkers <- read.table (file="filtered-names-markers.tbl", stringsAsFactors=F)[,1]
	filteredSamples <- read.table (file="filtered-names-samples.tbl", stringsAsFactors=F)[,1]
	...
	outGenoFile  = "filtered-gwasp4-genotype.tbl"
	outPhenoFile = "filtered-gwasp4-phenotype.tbl"
	...
	# Write plink pheno filtered
	gwaspPhenoToPlinkPheno (outPhenoFile,"filtered-plink-phenotype.tbl") 
...
showResults <- function (data4, testModels, trait, gwasModel, phenotypeFile, snpsAnnFile, ploidy) 
{
	...
	msg (">>>>", "Writing results...")
	outFile = sprintf ("out-gwasp%s-%s-QTLs.tbl", ploidy, gwasModel) 
