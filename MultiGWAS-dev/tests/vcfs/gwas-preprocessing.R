...
#------------------------------------------------------------------------------
convertVcfToAcgtUsingNgsep <- function (filename, outFilename="") {
...
#------------------------------------------------------------------------------
# Convert VCF to ACGT files using R "vcfR"
#------------------------------------------------------------------------------
RconvertVCFtoACGT <- function (filename, outFilename="") {
...
#------------------------------------------------------------------------------
plinkToVCFFormat <- function (plinkFile, outFilename="") {
	if (outFilename=="")
		outFilename = paste0 (strsplit (plinkFile, "[.]")[[1]][1], "-VCF")

	#plinkPrefix = strsplit (plinkFile, split="[.]")[[1]][1]
	cmm = sprintf ("plink --file %s --recode vcf-fid --out %s", plinkFile, outFilename)
	runCommand (cmm)
}
...
gwaspToPlinkFormat <- function (genotypeFile, plinkFile="") {
	if (plinkFile=="")
		plinkFile = paste0 (strsplit (genotypeFile, "[.]")[[1]][1], "-PLINK")
...
{
	args = commandArgs(trailingOnly=T)
	#args = c ("multisample.vcf")

	vcf  = args [1]

	convertVcfToAcgtUsingNgsep (vcf)


	#gwaspToTasselPhenotype (pheno, "filt-tassel-pheno.tbl")
	#plinkToVCFFormat ("example-genotype-tetra-PLINK")
